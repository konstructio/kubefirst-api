...
// Kubefirst-specific pinning
replace (
	// There are several newer versions of this package including a stable v1, but
	// we use it in a few places and some of the parameters are missing (like `zoneID`).
	// We should consider updating this package in the future to v1 stable.
	github.com/cloudflare/cloudflare-go => github.com/cloudflare/cloudflare-go v0.73.0

	// SSH key pagination is currently broken. The endpoint is paginated but version
	// v0.81.0 of the library does not support pagination. We need to update.

	...
	return filepath.WalkFunc(func(path string, fi os.FileInfo, err error) error {
		...
		if matched {
			...
			if !strings.Contains(path, "/.git/") {
				...
				// Azure
				newContents = strings.Replace(newContents, "<KUBEFIRST_STATE_STORE_RESOURCE_GROUP>", tokens.AzureStorageResourceGroup, -1)
				newContents = strings.Replace(newContents, "<KUBEFIRST_STATE_STORE_CONTAINER_NAME>", tokens.AzureStorageContainerName, -1)
